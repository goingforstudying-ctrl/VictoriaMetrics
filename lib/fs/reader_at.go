...
var disableFadviseRandomRead = flag.Bool("fs.disableFadviseRandomRead", false, "Whether to disable fadvise(FADV_RANDOM) hint "+
	...
	"This reduces the amount of unneeded data read from disk during queries, which select small number of blocks scattered across big data files. "+
	"Disabling the hint may improve performance for queries, which read the most of the data in big data files")
...
// so the OS must skip readahead for reads via pread() syscall at mustReadAtViaSyscall.

...
* FEATURE: all VictoriaMetrics components: expose `process_cpu_cores_cgroup_limit` and `process_memory_cgroup_limit_bytes` metrics. These metrics report the configured cgroup CPU and memory limits, or `0` when no finite corresponding cgroup limit can be detected.

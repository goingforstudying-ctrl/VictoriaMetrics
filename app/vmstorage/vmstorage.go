...
func (vms *VMStorage) InitSearch(ctx *storage.RequestContext, qt *querytracer.Tracer, sq *storage.SearchQuery) (vmselectapi.BlockIterator, error) {
	return vms.initSearch(ctx, qt, sq, marshalDefault)
}

func (vms *VMStorage) initSearch(ctx *storage.RequestContext, qt *querytracer.Tracer, sq *storage.SearchQuery, marshal marshalFunc) (vmselectapi.BlockIterator, error) {
	tr := sq.GetTimeRange()
...
func (bi *blockIterator) NextBlock(ctx *storage.RequestContext, dst []byte) ([]byte, bool) {
...
func (vms *VMStorage) SearchMetricNames(ctx *storage.RequestContext, qt *querytracer.Tracer, sq *storage.SearchQuery) ([]string, error) {
	tr := sq.GetTimeRange()
...
// tr.
func (vms *VMStorage) LabelValues(ctx *storage.RequestContext, qt *querytracer.Tracer, sq *storage.SearchQuery, labelName string, maxLabelValues int) ([]string, error) {
	tr := sq.GetTimeRange()
...
// similar APIs.
func (vms *VMStorage) TagValueSuffixes(ctx *storage.RequestContext, qt *querytracer.Tracer, accountID, projectID uint32, tr storage.TimeRange, tagKey, tagValuePrefix string, delimiter byte,
...
func (vms *VMStorage) LabelNames(ctx *storage.RequestContext, qt *querytracer.Tracer, sq *storage.SearchQuery, maxLabelNames int) ([]string, error) {
	tr := sq.GetTimeRange()
...
func (vms *VMStorage) SeriesCount(ctx *storage.RequestContext, qt *querytracer.Tracer, accountID, projectID uint32) (uint64, error) {
	return vms.s.GetSeriesCount(ctx, accountID, projectID)
}

func (vms *VMStorage) Tenants(ctx *storage.RequestContext, qt *querytracer.Tracer, tr storage.TimeRange) ([]string, error) {
	return vms.s.SearchTenants(ctx, qt, tr)
}

// GetTSDBStatus returns TSDB status for given filters on the given date.
func (vms *VMStorage) TSDBStatus(ctx *storage.RequestContext, qt *querytracer.Tracer, sq *storage.SearchQuery, focusLabel string, topN int) (*storage.TSDBStatus, error) {
	tr := sq.GetTimeRange()
	maxMetrics := sq.MaxMetrics
	if maxMetrics <= 0 {
		// fallback to maxUniqueTimeSeries if no limit is provided,
...
func (vms *VMStorage) DeleteSeries(ctx *storage.RequestContext, qt *querytracer.Tracer, sq *storage.SearchQuery) (int, error) {
	tr := sq.GetTimeRange()
	maxMetrics := sq.MaxMetrics
	if maxMetrics <= 0 {
		// fallback to maxUniqueTimeSeries if no limit is provided,
...
func (vms *VMStorage) RegisterMetricNames(ctx *storage.RequestContext, qt *querytracer.Tracer, mrs []storage.MetricRow) error {
	vms.s.RegisterMetricNames(qt, mrs)
	return nil
}

// GetMetricNamesUsageStats returns metric name usage stats.
func (vms *VMStorage) GetMetricNamesUsageStats(ctx *storage.RequestContext, qt *querytracer.Tracer, tt *storage.TenantToken, limit, le int, matchPattern string) (metricnamestats.StatsResult, error) {
...
func (vms *VMStorage) ResetMetricNamesUsageStats(ctx *storage.RequestContext, qt *querytracer.Tracer) error {
	vms.s.ResetMetricNamesStats(qt)
	return nil
}

func (vms *VMStorage) setupTfss(ctx *storage.RequestContext, qt *querytracer.Tracer, sq *storage.SearchQuery, tr storage.TimeRange, maxMetrics int) ([]*storage.TagFilters, error) {
...
func (vms *VMStorage) GetMetadataRecords(ctx *storage.RequestContext, qt *querytracer.Tracer, tt *storage.TenantToken, limit int, metricName string) ([]*metricsmetadata.Row, error) {
